...
<html>
	...
	<body>
		...
		<form id="categoryForm" action="Category2Servlet?action=getPageByQuery" method="post">
		...
		<div class="text-center categoryTableDiv">
			...
			<table id="categoryTable" class="table table-striped table-bordered table-hover">
				<thead>
					<tr class="tableHead">
						<th width="10%">商品种类编号</th>
						<th width="15%">商品种类名称</th>
						<th width="25%">商品种类描述</th>
						<th width="25%">所属种类</th>
						<th width="10%">操作</th>
						<th width="10%">操作</th>
					</tr>
			...
			<div id="tiaozhuan">
			<form id="categoryForm2" action="Category2Servlet?action=getPageByQuery" method="post">
				<input type="hidden" name="searchCondition" value='${requestScope.searchCondition}' class="form-control"/>
				<a id="first" class="btn btn-default btn-sm" href="Category2Servlet?action=getPageByQuery&requestPage=1&searchCondition=${requestScope.searchCondition}">首页</a>
				<a id="previous" class="btn btn-default btn-sm" href="Category2Servlet?action=getPageByQuery&requestPage=<%=pageInfo.getPreviousPage()%>&searchCondition=${requestScope.searchCondition}"> 上一页</a>
				
				<a id="next" class="btn btn-default btn-sm" href="Category2Servlet?action=getPageByQuery&requestPage=<%=pageInfo.getNextPage()%>&searchCondition=${requestScope.searchCondition}"> 下一页</a>
				  <span class="mySpan">
				  <a id="last" class="btn btn-default btn-sm" href="Category2Servlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>">尾页</a>
    ...
    <span>页 </span>
		...
		<script type="text/javascript">
			...
			function deleteById(id){
				...
				$.confirm({
					...
					confirm: function() {
						...
						location.assign("Category2Servlet?action=delete&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&cid=" + id);
			...
			function getOneByIdUpdate(id){
				location.assign("Category2Servlet?action=getOne&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&cid=" + id);
