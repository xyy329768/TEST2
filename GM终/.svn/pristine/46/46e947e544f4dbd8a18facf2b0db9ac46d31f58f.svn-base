...
public class EasyServlet extends HttpServlet {
	...
	public static MimeMessage createMimeMessage(HttpSession session1, Session session, String sendMail, String receiveMail) throws Exception {		
		...
		message.setFrom(new InternetAddress(sendMail, "²ÜËÉ", "UTF-8"));
	    message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "ÄÄß¸", "UTF-8"));
	    message.setSubject("×¢²á¼¤»î", "UTF-8");
	    message.setContent("<a href=\"http://localhost:8080/gm/UserServlet?action=update&username="+ users +"\">Á¢¼´¼¤»î</a>", "text/html;charset=UTF-8");
