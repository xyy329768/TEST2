<%@ page language="java" import="java.util.*,com.vo.*,com.page.PageInfo,java.text.*" pageEncoding="UTF-8"%>
<%
...
<html>
	...
	<body>
		...
		<form id="categoryForm" action="OrderServlet?action=getOrderByQuery&target=productMain" method="post">
		...
		<div class="text-center categoryTableDiv">
			...
			<div id="tiaozhuan">
			<form id="categoryForm2" action="OrderServlet?action=getOrderByQuery&target=productMain" method="post">
				<input type="hidden" name="searchCondition" value='${requestScope.searchCondition}' class="form-control"/>
				
				<a id="first" class="btn btn-default btn-sm" href="OrderServlet?action=getOrderByQuery&requestPage=1&target=productMain&searchCondition=${requestScope.searchCondition}">首页</a>
				<a id="previous" class="btn btn-default btn-sm" href="OrderServlet?action=getOrderByQuery&target=productMain&requestPage=<%=pageInfo.getPreviousPage()%>&searchCondition=${requestScope.searchCondition}"> 上一页</a>
				
				<a id="next" class="btn btn-default btn-sm" href="OrderServlet?action=getOrderByQuery&target=productMain&requestPage=<%=pageInfo.getNextPage()%>&searchCondition=${requestScope.searchCondition}"> 下一页</a>
				  <span class="mySpan">
				  <a id="last" class="btn btn-default btn-sm" href="OrderServlet?action=getOrderByQuery&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>">尾页</a>
