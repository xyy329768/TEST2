...
<html>
  ...
  <body>
	...
	<form id="user1Form2" action="User1Servlet?action=getPageByQuery&target=UserMain" method="post">
	...
	<div class="text-center userTableDiv">
			...
			<tbody>
				...
				%>
					...
					<tr>
						...
						<td>
							<button class="btn btn-success btn-xs" onclick="getOneForUpdate(<%=user1.getUserId() %>)">修改</button>
						</td>
						<td>
							<button class="btn btn-danger btn-xs" onclick="deleteUser1ById(<%=user1.getUserId() %>)">删除</button>
						</td>
					</tr>
		...
		<div id="pageDiv">
			...
			<form id="user1Form" action="User1Servlet?action=getPageByQuery" method="post">
			<input type="hidden" name="searchCondition" value="${requestScope.searchCondition}"/>
	<a id="first" class="btn btn-default btn-sm" href="User1Servlet?action=getPageByQuery&target=UserMain&searchCondition=${requestScope.searchCondition}&requestPage=1">首页</a>
    
    
    <a id="previous" class="btn btn-default btn-sm" href="User1Servlet?action=getPageByQuery&target=UserMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>"> 上一页</a>
      <a id="next" class="btn btn-default btn-sm" href="User1Servlet?action=getPageByQuery&target=UserMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>"> 下一页</a>
    
    
     <a id="last" class="btn btn-default btn-sm" href="User1Servlet?action=getPageByQuery&target=UserMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>">尾页</a>
   ...
   <span>共<%=pageInfo.getTotalRecordCount()%>条记录 </span>
	...
	<script type="text/javascript">
		...
		function getOneForUpdate(id){
			...
			location.assign("User1Servlet?action=getOneForUpdate&target=UserMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&userId=" + id);
		}
		
		
		function deleteUser1ById(id){
			...
			$.confirm({
				...
				confirm: function() {
					...
					location.assign("User1Servlet?action=delete&target=UserMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&userId=" + id);
				},
				cancel: function() {
